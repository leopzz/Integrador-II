...
function Entrada() {
    ...
    const CarregarGrid = async (dados) => {
        await axios.post("http://localhost:3000/Entradas/ObterGridPesquisa", dados).then((res) => {
    ...
    const salvarClick = async () => {
        var dados = {
            id_entrada: id,
            ds_entrada: descricao,
            vl_preco: valor,
            sn_ativo: status
        }
        console.log(dados)
        await axios.post("http://localhost:3000/Entradas/Salvar", dados).then((res) => {
    ...
    const preencherCadastro = async (e) => {
        ...
        await axios.post("http://localhost:3000/Entradas/BuscarPorCodigo", { Codigo: e.target.id }).then((res) => {
            var data = res.data.Data;
            setDescricao(data.ds_entrada);
            setStatus(data.sn_ativo);
            setId(data.id_entrada);
                ...
                <Col md={10} style={{ justifyContent: "center", display: "flex" }}>
                    <Col md={11}>
                        ...
                        <Row md={12} style={{ marginBottom: "2rem", marginTop: "1rem" }}>
                            <h2 style={{ marginBottom: "1rem", color: "#fff" }}>Entradas</h2>
